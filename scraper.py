...
def extract_with_gemini(html, firm_name, api_key):
    try:
        ...
        model = None
        for model_name in ["gemini-pro", "gemini-1.5-flash-latest", "gemini-1.5-pro-latest", "gemini-1.0-pro"]:
            try:
...
- Do NOT include: sector labels, regions, years, fund names, investment status words
- Do NOT include: navigation text, button labels, descriptions, headers, footers  
- Do NOT include the firm name "{firm_name}" itself
- Do NOT add numbers, bullets, dashes or any prefix before names
- If you find no company names at all, reply with just the word: NONE
...
def find_firm_website(firm_name):
    try:
        ...
        skip = ['wikipedia','linkedin','crunchbase','bloomberg','economictimes',
                ...
                'facebook','instagram','youtube','inc42','businesstoday','livemint','forbes']
        with DDGS() as ddgs:
...
async def scrape_portfolio(firm_name, api_key, status_widget=None):
    ...
    if db:
        ...
        update(f"Step 1/3 — Found {firm_name} in database, opening portfolio page...")
    else:
    ...
    update(f"Step 2/3 — Loading portfolio page and reading content...")
    ...
    update(f"Step 3/3 — Extracting company names with Gemini AI...")
